...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// base case -- if this page is a leaf node in the tree
		if(pid.pgcateg() == BTreePageId.LEAF){
			// get current page & return it
			BTreeLeafPage leaf = (BTreeLeafPage) this.getPage(tid, dirtypages, pid, perm);
			return leaf;
		}
		// else...
		BTreeInternalPage pg = (BTreeInternalPage) this.getPage(tid, dirtypages, pid, perm);
		Iterator<BTreeEntry> bTreeIter = pg.iterator();
		BTreeEntry node = bTreeIter.next(); // advance once
		if(node == null){
			throw new DbException("Empty internal page!");
		}

		if(f == null){
			// if f is null, find the left-most leaf page & return that
			BTreeLeafPage leftmost = this.findLeafPage(tid, dirtypages, node.getLeftChild(), perm, null);
			return leftmost;
		}

		// at this point we are not yet at a leaf node, f is not null, and the node is not null
		while(bTreeIter.hasNext()){
			// advance again
			BTreeEntry current = bTreeIter.next();
			// if the node we are on (current) is less than node, update node so that it is now current
			if(current.getKey().compare(Op.LESS_THAN, f)){
				node = current;
			} else{ // node is less than current, so no update needed
				break;
			}
		}

		// branch off into the node's children...
		boolean branch = f.compare(Op.GREATER_THAN_OR_EQ, node.getKey());
		BTreePageId child;
		if (branch){ // >=
		...
		// recursive call to go to the child node
		return this.findLeafPage(tid, dirtypages, child, perm, f);
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		// new leaf page we are splitting into
		BTreeLeafPage newLeaf = (BTreeLeafPage) this.getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		// iterator to move tuples over into new page
		BTreeLeafPageIterator btIter = (BTreeLeafPageIterator) page.iterator();

		// get middle tuple
		int i = 0;
		int m = page.getNumTuples()/2;
		while(i< m & btIter.hasNext()){
			btIter.next();
			i++;
		}

		// while the iterator has a next, grab a tuple & delete it from the current
		// page it's in & add it into the new page we previously created
		while (btIter.hasNext()){
			Tuple current = btIter.next();
			page.deleteTuple(current);
			newLeaf.insertTuple(current);
		}

		// grab the middle tuple
		Field middle = newLeaf.iterator().next().getField(this.keyField());
		// the parent page
		BTreeInternalPage parent = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), middle);
		// updating the parentID of the new page we created
		newLeaf.setParentId(parent.getId());

		// inserting middle key into the parent page
		BTreeEntry middleKey = new BTreeEntry(middle, page.getId(), newLeaf.getId());
		parent.insertEntry(middleKey);

		// updating relationships...
		// updating newLeaf's relationships
		newLeaf.setRightSiblingId(page.getRightSiblingId());
		newLeaf.setLeftSiblingId(page.getId());
		newLeaf.setParentId(parent.getId());

		// if a right sibling for newLeaf exists, update the pointer to have newLeaf as
		// its left sibling
		if (newLeaf.getRightSiblingId() != null) {
			BTreeLeafPage rightNew = (BTreeLeafPage) this.getPage(tid, dirtypages, newLeaf.getRightSiblingId(), Permissions.READ_WRITE);
			rightNew.setLeftSiblingId(newLeaf.getId());
			dirtypages.put(rightNew.getId(), rightNew);
		}

		// updating existing page's right sibling to be the new page we created
		...
		if (field.compare(Op.GREATER_THAN, middle)){
			return newLeaf;
		}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// the new internal page
		BTreeInternalPage newInternal = (BTreeInternalPage) this.getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);

		// find the middle entry
		int middle = page.getNumEntries() / 2;
		// iterator to go through tuples
		BTreeInternalPageIterator internalIter = (BTreeInternalPageIterator) page.iterator();

		// go through the tuples & stop right before the middle entry
		int i = 0;
		while(i < middle && internalIter.hasNext()){
			internalIter.next();
			i++;
		}

		// get the middle entry
		BTreeEntry middleEntry = internalIter.next();

		// delete the middle entry
		page.deleteKeyAndRightChild(middleEntry);
		// getting the tuples to the right of the middle entry
		while(internalIter.hasNext()){
			BTreeEntry j = internalIter.next();
			page.deleteKeyAndRightChild(j);
			newInternal.insertEntry(j);
		}

		// update parent pointers w/ newInternal
		this.updateParentPointers(tid, dirtypages, newInternal);
		...
		middleEntry.setRightChild(newInternal.getId());
		parent.insertEntry(middleEntry);
		newInternal.setParentId(parent.getId());
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// label page as 'right' if applicable -- will be useful to find
		// where out iterator should start at
		if(isRightSibling){ // if true, then page is LEFT page
			left = page;
			right = sibling;
		} else{	// if false, then page is RIGHT page
			left = sibling;
			right = page;
		}

		// initialize iterator in left page
		BTreeLeafPageIterator iter = (BTreeLeafPageIterator) left.iterator();

		// advance iterator to 'middle' tuple of both pages
		int pos = 0;
		int middle = (left.getNumTuples() + right.getNumTuples())/2;
		while(pos < middle && iter.hasNext()){
			pos += 1;
			iter.next();
		}

		// stealing the elements now...
		// if iter.hasNext() then that means our left page has more
		// elements than our right page
		if(iter.hasNext()){
			// steal the rest of the elements from left page and add to right page
			while(iter.hasNext()){
				Tuple i = iter.next();
				left.deleteTuple(i);
				right.insertTuple(i);
			...
			// steal from right page and add to left page
			BTreeLeafPageIterator rIter = (BTreeLeafPageIterator) right.iterator();
			int i = pos;
			while(i<middle && rIter.hasNext()){
				Tuple j = rIter.next();
				right.deleteTuple(j);
				left.insertTuple(j);
				i++;
		...
		Tuple rNext = right.iterator().next();
		entry.setKey(rNext.getField(this.keyField()));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// get the right child of the left sibling & the left child of the current page
		// this will help determine which page we are stealing from, etc.
		BTreePageId leftSibRightChild = leftSibling.reverseIterator().next().getLeftChild();
		BTreePageId leftChild = page.iterator().next().getLeftChild();

		// add the new entry to current page (we are stealing from left)
		BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), leftSibRightChild, leftChild);
		page.insertEntry(newEntry);

		// advance iterator to correct spot
		int numDiff = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
		int i = 0;
		BTreeInternalPageReverseIterator lIter = (BTreeInternalPageReverseIterator) leftSibling.reverseIterator();
		while (lIter.hasNext() && i < numDiff){
			newEntry = lIter.next();
			leftSibling.deleteKeyAndRightChild(newEntry);
			page.insertEntry(newEntry);
			i++;
		}
		...
		if (lIter.hasNext()) {
			newEntry = lIter.next();
			leftSibling.deleteKeyAndRightChild(newEntry);
			parentEntry.setKey(newEntry.getKey());
			parent.updateEntry(parentEntry);

			// update parent pointers!!
			this.updateParentPointers(tid, dirtypages, page);

			// update dirty pages like before...
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// similar to steal from left internal page
		// assign correct variables / pointers
		BTreePageId rightSibLeftChild = rightSibling.iterator().next().getLeftChild();
		BTreePageId rightChild = page.reverseIterator().next().getRightChild();

		// ner entry from right page to current page
		BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), rightChild, rightSibLeftChild);
		page.insertEntry(newEntry);

		// advance iterator to right spot
		int numDiff = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
		int i = 0;
		BTreeInternalPageIterator rIter = (BTreeInternalPageIterator) rightSibling.iterator();
		while (rIter.hasNext() && i < numDiff){
			newEntry = rIter.next();
			rightSibling.deleteKeyAndLeftChild(newEntry);
			page.insertEntry(newEntry);
			i++;
		}
